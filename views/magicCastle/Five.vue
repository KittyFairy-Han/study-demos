	...
	<section class="five-prizes" :class="useAnimation ? 'use-animation' : ''">
		...
		<ul class="five-prizes-container">
			...
			<li class="prize-item" v-for="(item, index) in prizes" :key="index">
				<div class="pic" :class="isRarePrize(item) ? 'rare' : ''">
					<img :src="item.imgUrl" />
				</div>
...
<script>
import { isRarePrize } from '../../common/util.js'
...
.five-prizes {
	...
	.text {
		...
		font-size: 32px;
		color: #f9e1ff;
		text-shadow: 0px 0px 11.9975px rgba(255, 255, 255, 0.6),
			0px 0px 23.9951px rgba(255, 255, 255, 0.25);
	}
	ul {
		position: relative;
		width: 100%;
		...
		.prize-item {
			...
			.pic {
				.el-fit-bgimg(240px,240px,url('./images/prize-blue.png'));
				display: flex;
				justify-content: center;
				align-items: center;

				&.rare {
					.el-fit-bgimg(240px,240px,url('./images/prize-gold.png'));
				}
				img {
					width: 160px;
					height: 160px;
